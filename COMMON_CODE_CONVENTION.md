...
- Блок `catch` должен логгировать ошибку которая произошла, и при необходимо пробрасывать **кастомное** исключение вверх по стэку.
- Избегайте вложенности, используйте подход **Guard Clauses** при котором выражение проверяется не на истинность, а на ложь, и досрочно выходит из функции.
- Избегайте вложенности, если используется создание объекта который реализует интерфейс `IDisposable` используйте `using` без создания тела.
